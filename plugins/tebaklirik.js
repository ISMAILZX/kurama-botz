...
`.trim()
    conn.tebaklirik[id] = [
        await conn.sendButton(m.chat, caption, '© IsmailKurama', 'Bantuan', `.teli`, m),
        json, poin,
        setTimeout(async () => {
            if (conn.tebaklirik[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.jawaban}*`, '© IsmailKurama', 'Tebak Lirik', `.tebaklirik`, conn.tebaklirik[id][0])
